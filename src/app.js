"use strict";
/* Data arrives from this app's own API, which proxies NOAA and the NWS.
   WATER is keyed by date and holds tide and current; WIND holds the hourly
   forecast for the sail date. Both are filled in the background. */
...
async function loadDay(ds) {
  if (!ds) return;
  const days = [addDays(ds, -1), ds, addDays(ds, 1)];
  const jobs = [];
  days.forEach(d => {
    if (WATER[d] !== undefined || pending['w' + d]) return;
    pending['w' + d] = true;
    jobs.push(fetch('api/water?date=' + d)
      .then(r => r.ok ? r.json() : Promise.reject(r.status))
      .then(j => { WATER[d] = j; })
      .catch(() => { WATER[d] = null; })
      .finally(() => { delete pending['w' + d]; }));
  });
  if (WIND[ds] === undefined && !pending['n' + ds]) {
    pending['n' + ds] = true;
    jobs.push(fetch('api/wind?date=' + ds)
      .then(r => r.ok ? r.json() : Promise.reject(r.status))
      .then(j => { WIND[ds] = j; })
      .catch(() => { WIND[ds] = null; })
      .finally(() => { delete pending['n' + ds]; }));
  }
...
function autofillWind(ds) {
  ...
  if (!w || !w.hours || !w.hours.length) {
    ...
    line.textContent = WIND[ds] === null
      ? 'The forecast did not load. Open "Use a different forecast" below, or give the values yourself.'
      : 'The NWS has no forecast for that date. It reaches about 7 days ahead. Give the wind values yourself, or paste a forecast below.';
    return;
  }
...
function renderRose(A) {
  ...
  txt.innerHTML = 'The solid arrow shows the ' + nm + '. It sets <strong>' + setDir(A.vStart) + '\u00B0</strong> at ' + hhmm(I.dep) +
    '. The dashed arrow shows where the wind pushes you. The wind is from ' + compass(I.windDir) + '. ' +
