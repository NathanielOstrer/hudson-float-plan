...
function buildPlan(A) {
  ...
  if (phases.length === 1) {
    ...
    /* the band can start before cast off or end after the return, and a step
       outside the window would print out of order */
    const beganBefore = S.early <= I.dep + 10, endsAfter = S.late >= I.ret - 10;
    if (spread && beganBefore) {
      lead = 'The current is turning at cast off. It is ' + b.name + ' everywhere by ' + hhmm(S.late) + '. Go ' + axis(a.dir) + ' first. Ride the ' + b.name + ' home.';
      why = 'The turn is under way when you cast off. The deep water and the bank turn first. The surface turns last. ' +
        'The ' + a.name + ' is weak, so do not go far ' + axis(a.dir) + '. The new ' + b.name + ' then pushes you back to Pier 66.';
    } else {
    ...
    steps.push([hhmm(I.dep), 'Cast off. Go <em>' + axis(a.dir) + '</em>, ' + toward(a.dir) + '. The ' + a.name + ' gives you up to ' + Math.abs(a.peak).toFixed(1) + ' kt.'
      + (spread && beganBefore ? ' The deep water is already turning to the ' + b.name + '.' : '')]);
    if (S.early - 20 > I.dep + 10) steps.push([hhmm(S.early - 20), 'Turn back. The current is weak from this time.']);
    if (spread) {
      if (!beganBefore) steps.push([hhmm(S.early), 'Slack water begins. The deep water and the bank turn first.']);
      if (!endsAfter) steps.push([hhmm(S.late), 'Slack water ends. The surface turns last. Expect the ' + b.name + ' everywhere from here.']);
    } else {
    ...
    steps.push([hhmm(I.ret), 'Come alongside. The ' + b.name + ' gives you up to ' + Math.abs(b.peak).toFixed(1) + ' kt. Approach into it.'
      + (spread && endsAfter ? ' The surface is still turning until ' + hhmm(S.late) + '. Expect weak current at the dock.' : '')]);
  } else {
    ...
    slacks.forEach((sl, i) => steps.push([hhmm(Math.max(sl.early, I.dep + 5)),
